...
X_train =df_train[:]
X_test = df_test[:]
'''df_not = df_all[df_all['relevance'] != 3]
num_tr2 = df_train[df_train['relevance'] != 3].shape[0]
df_tr2 = df_all.iloc[:num_tr2]
y_tr2 = df_tr2['relevance'].values
X_tr2 =df_tr2[:]'''
#functions for model
...
clf = pipeline.Pipeline([
        ...
        ('rfr', rfr)])
param_grid = {'rfr__max_features': [18], 'rfr__max_depth': [20]}
...
model.best_params_
#labeldummy, randomfrst = model.best_estimator_.steps[-1]
#myimportances = randomfrst.feature_importances_
#randomfrst.estimators_[:10]
